...
- 8d92337: Refactor the package around an explicit, composable ESLint flat-config API.
...
- 30e58fa: Drop `@eslint/core` dependency, use built-in `Linter.RuleEntry` type from ESLint instead of `RuleConfig` for rule severity handling.
...
- ae4fd8d: Restore and undeprecate `@nelsonlaidev/eslint-config`, and refresh the shared preset against the previous `3.7.1` release.
...
- b77d673: Update ESLint plugin dependencies and migrate `eslint-plugin-better-tailwindcss` to v4.3.1 API
...
- 4bf1bfd: Add `noUnassignedImport` option to `import-x` config to allow custom glob patterns in the `no-unassigned-import` rule. This follows the same pattern as the existing `noNamespace` option, enabling users to extend the default allowed patterns for unassigned imports.
